...
You just imported the inputs generator for the `SiestaCalculation`. We can explore its functionality::
...
We got a builder ready to be submitted. look for instance at::

        builder.parameters.attributes

We can add the spin polarization with::
...
Close the shell and look at the file ``my_protocols_registry.yaml``. It contains a new set of inputs and the psf pseudos. Do:: 
...
The new protocol is on the list and we can use it to submit a calculation.
