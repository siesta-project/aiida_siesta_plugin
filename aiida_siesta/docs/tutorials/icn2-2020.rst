...
Reload the environment and check the status of the installation::
...
set up of a pseudopotential family. We will download a set of
pseudopotentials from `PseudoDojo <http://www.pseudo-dojo.org/>`_ and store them in the database.
Here is the code::
