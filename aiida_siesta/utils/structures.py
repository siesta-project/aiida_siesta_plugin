...
def find_intermediate_structure(s, i1, i2, intermediate_position):
    """
    Given a structure and the indexes of two sites, and an intermediate
    point, generate an intermediate structure in which the i1 atom
    has moved to the intermediate point and the second atom has moved
    to an image point with an opposite relative displacement. This is
    useful to 'prime' a path to avoid head-on collissions, for example.
    ...
    # Now, the relative position of the path point and the first atom
