        # In Siesta's case, for now, it is just the density-matrix fil2
# -*- coding: utf-8 -*-
import os
...
class FCBuildCalculation(JobCalculation):
    """
    Plugin for the "fcbuild" program in the Siesta-Vibra distribution, which
    builds the supercell required for a phonon calculation.
    """
    _siesta_plugin_version = 'aiida-0.11.0--plugin-0.11.5'

    def _init_internal_params(self):
        super(FCBuildCalculation, self)._init_internal_params()

        # Default Siesta output parser provided by AiiDA
        #self._default_parser = "siesta.fcbuild"
    ...
    def _prepare_for_submission(self, tempfolder, inputdict):
        ...
        # ------------- ATOMIC_SPECIES ------------
        # Only the species index is necessary
        ...
        for site in structure.sites:
            ...
            atomic_positions_card_list.append(
                "{0:18.10f} {1:18.10f} {2:18.10f} {3:4} {4:5}\n".format(
                    site.position[0], site.position[1], site.position[2],
                    spind[site.kind_name], countatm))
