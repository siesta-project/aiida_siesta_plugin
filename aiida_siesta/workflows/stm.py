...
class SiestaSTMWorkChain(WorkChain):
    ...
    def checks(self):
        ...
        for k, v in sorted(translatedkey.get_filtered_items()):
            if k=="%block localdensityofstates":
                ...
                self.report('WARNING: A local-density-of-state block was defined in the input parameters, however '
    ...
    def run_siesta_with_ldos(self):
        ...
        ef = self.ctx.workchain_base.outputs.output_parameters["E_Fermi"]
        okemax = self.inputs.emax.value - ef
        okemin = self.inputs.emin.value - ef
    ...
    def run_results(self):
        ...
        if self.ctx.workchain_base.outputs.output_parameters.attributes["variable_geometry"]:
            output_structure = self.ctx.siesta_ldos.outputs.output_structure ##CAREFULL, if strip relax opt, this must be mdified
